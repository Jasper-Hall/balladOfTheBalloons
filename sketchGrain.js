...
async function loadInitialData() {
    try {
        const response = await fetch('./public/radiosondeDataPreview.json');  
        if (!response.ok) {
  ...
  async function fetchRadiosondeData() {
    try {
        ...
        if (false) { // Removed devMode check
            console.log("Dev mode: using local sample data");
            const localResponse = await fetch('./public/radiosondeDataPreview.json');
            if (localResponse.ok) {
                const localData = await localResponse.json();
                radiosondes = localData;
                console.log("Loaded local sample data:", radiosondes);
        ...
        for (const proxy of corsProxies) {
            try {
                const url = proxy + 'https://api.v2.sondehub.org/sondes/telemetry?duration=12h';
                console.log("Trying proxy:", proxy);
                response = await fetch(url);
                
                if (response.ok) {
                    console.log("Proxy succeeded:", proxy, "Status:", response.status);
                    break; // Success, exit the loop
                } else {
                    console.log("Proxy failed with status:", proxy, response.status, response.statusText);
                }
            } catch (error) {
        ...
        if (data.contents) {
            ...
            radiosondeData = data.contents;
        } else if (data.data) {
            // Some proxies wrap in a 'data' field
            radiosondeData = data.data;
        } else {
            // Direct response
            radiosondeData = data;
        }
        ...
        if (typeof radiosondeData === 'string') {
            radiosondes = JSON.parse(radiosondeData);
        } else {
            radiosondes = radiosondeData;
        }
        
        console.log("Processed radiosonde data:", radiosondes);
        ...
        try {
            ...
            if (localResponse.ok) {
                const localData = await localResponse.json();
                radiosondes = localData;
                console.log("Using local sample data:", radiosondes);
...
function preload() {
  sourceFile = loadSound('./public/upupandaway.mp3', soundLoaded);
  reverbIR = loadSound('./public/IRx500_01A.wav')
}
...
function drawMap() {
    ...
    // Uniform subtle grid lines (no majors)
    stroke(34, 58, 150, 30); // subtle blue
    strokeWeight(0.5);
...
function windowResized() {
  ...
  if (!holderEl) return;
  const holderRect = holderEl.getBoundingClientRect();
  const newWidth = Math.max(100, holderRect.width);
  const newHeight = Math.max(100, holderRect.height);
